...
def _find_modules_v2( # actually v3 add LoraInjectedConv3d
    model,
...
def inject_trainable_lora_extended3d(
    model: nn.Module,
    target_replace_module: Set[str] = UNET_EXTENDED_TARGET_REPLACE,
    r: int = 4,
    loras=None,                                                     # path to lora .pt
...
def parse_safeloras(safeloras,) -> Dict[str, Tuple[List[nn.parameter.Parameter], List[int], List[str]]]:
    """
...
def monkeypatch_remove_lora(model):
    for _module, name, _child_module in _find_modules(model, search_class=[LoraInjectedLinear, LoraInjectedConv2d, LoraInjectedConv3d]):
        if isinstance(_child_module, LoraInjectedLinear):
        ...
        elif isinstance(_child_module, LoraInjectedConv2d):
            ...
            if bias is not None:
                _tmp.bias = bias
        elif isinstance(_child_module, LoraInjectedConv3d):
            _source = _child_module.conv
            weight, bias = _source.weight, _source.bias
