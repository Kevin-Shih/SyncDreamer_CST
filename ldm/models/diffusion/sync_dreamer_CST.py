...
class SpatialVolumeNet(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_volume_feats = SpatialTime3DNet(
            input_dim=16 * view_num, time_dim=time_dim, dims=(64, 128, 256, 512)
        )
    ...
    def construct_spatial_volume(self, x, t_embed, v_embed, target_poses, target_Ks):
        ...
        spatial_volume_verts = torch.linspace(
            -self.spatial_volume_length, self.spatial_volume_length, V, dtype=torch.float32, device=device
        )
        ...
        for ni in range(0, N):
            ...
            coords_source = get_warp_coordinates(
                spatial_volume_verts, x_.shape[-1], self.input_image_size, K_source_, pose_source_
            ).view(B, V, V * V, 2)
    ...
    def construct_view_frustum_volume(self, spatial_volume, t_embed, v_embed, poses, Ks, target_indices):
        ...
        near = torch.ones(
            B * TN, 1, H, W, dtype=spatial_volume.dtype, device=spatial_volume.device
        ...
        volume_feats = F.grid_sample(
            spatial_volume_, volume_xyz_, mode='bilinear', padding_mode='zeros', align_corners=True
        )                                                                                           # B*TN,C,D,H,W
...
class SyncMultiviewDiffusion(pl.LightningModule):
    ...
    def training_step(self, batch): # modify this for CST training our objective
        ...
        self.log(
            "step", self.global_step, prog_bar=True, logger=True, on_step=True, on_epoch=False, rank_zero_only=True
        )
        return loss
    ...
    def log_image(self, x_sample, batch, step, output_dir):
        ...
        for bi in range(B):
            img_pr_ = concat_images_list(
                process(batch['input_image'][bi]), *[process(x_sample[bi, ni].permute(1, 2, 0)) for ni in range(N)]
            )
...
class SyncDDIMSampler:
    ...
    def __init__(
        self, model: SyncMultiviewDiffusion, ddim_num_steps, ddim_discretize="uniform", ddim_eta=1.0, latent_size=32
    ):
        super().__init__()
        self.model = model
    ...
    def _make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True):
        ...
        self.ddim_sigmas = ddim_eta * torch.sqrt((1 - self.ddim_alphas_prev) / (1 - self.ddim_alphas) *
        ...
        for ni in range(0, N, batch_view_num):
            ...
            if unconditional_scale != 1.0:
                noise = self.model.model.predict_with_unconditional_scale(
                    x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, unconditional_scale
                )
            else:
                noise = self.model.model(
                    x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, is_train=False
                )
