...
def render_gt_views(
    ...
    # azimuths = (np.arange(n) / n * np.pi * 2).astype(np.float32)     # modify to compensate training view
    # elevations = np.deg2rad(np.asarray([30] * n).astype(np.float32)) # modify to compensate training view

    azimuths = np.deg2rad((np.arange(n) / n * 360).astype(np.float32) - i_pose[1]) # modify to compensate training view
    elevations = np.deg2rad(np.asarray([30] * n).astype(np.float32))               # modify to compensate at gen_ifusion training view
    print(np.rad2deg(azimuths))
    print(np.rad2deg(elevations))
    ...
    for ni in tqdm(range(n)):
        ...
        # t = np.asarray([0, 0, 1.5 - (i_pose[2] - 1.5)])                   # modify to compensate training view
        # t = np.asarray([0, 0, 1.5 - (i_pose[2] - 1.5) * 1.5 / i_pose[2]]) # modify to compensate training view
        t = np.asarray([0, 0, 2.25 / i_pose[2]]) # modify to compensate training view
        ...
        imgs.append(image)
    imgs_16 = np.concat(imgs, 1)
    imgs_8 = np.concat(imgs[::2], 1)
    print(f'Saving to {output}/{outname[0]}')
    imageio.imsave(f'{output}/{outname[0]}', imgs_16)
    imageio.imsave(f'{output}/{outname[1]}', imgs_8)
...
def main():
    ...
    # parser.add_argument('-i', '--image_path', type=str, default='demo_0_16view.png')
    parser.add_argument('-i', '--image_idx', type=int, default='0')
    parser.add_argument('-t', '--transform_fp', type=str, default='transform.json')
    # parser.add_argument('-o', '--out', type=str, default='render_0_16view.png')
    ...
    log_dir = Path(opt.log) / f'neus_{opt.image_idx}'
    ckpt_dir = log_dir / 'ckpt'
    cfg.model.params['image_path'] = Path(opt.log) / f'demo_{opt.image_idx}_16view.png'
    ...
    callbacks.append(
        ModelCheckpoint(
            ...
            verbose=True,
            save_last=True,
            every_n_train_steps=callback_config.save_interval
        )
    )
    ...
    if opt.resume:
        ...
        model.load_state_dict(torch.load(str(ckpt_dir / 'last.ckpt'), weights_only=True))
    else:
        # if (ckpt_dir / 'last.ckpt').exists():
        #     raise RuntimeError(f"checkpoint {ckpt_dir / 'last.ckpt'} existing ...")
        trainer = Trainer.from_argparse_args(
            args=argparse.Namespace(), **trainer_config, logger=logger, callbacks=callbacks
        )

        trainer.fit(model, data)

    model = model.cuda().eval()
    torch.save(model.state_dict(), str(ckpt_dir / 'last.ckpt'))
    # render_images(model, log_dir)
    with open(opt.transform_fp, 'r') as f:
        transform = json.load(f)
        i_pose = transform['frames'][opt.image_idx]['latlon']
    render_gt_views(model, i_pose, Path(opt.log), [f'render_{opt.image_idx}_16view.png', f'render_{opt.image_idx}.png'])
